...
const DEFAULT_RULES = [
  ...
  '- Do not edit handoff `.md` contents. After the root alias is consumed (read + context built), delete `notion-handoff-<slug>.md` and/or `notion-handoff.md` from the repo root. Leave `.notion/handoffs/` archives alone. Remaining root aliases = unconsumed tickets.',
...
function buildIdeHandoffBody({
  ...
  return [
    ...
    'Branch handoff archives are kept in `.notion/handoffs/` and stable aliases are refreshed on each run.',
    '',
    'CONSUME AND CLEAN ROOT ALIAS (mandatory):',
    'After you have read this handoff and built context, delete the root alias file you attached (`notion-handoff-<slug>.md` and/or `notion-handoff.md`). Do not edit its contents. Do not delete `.notion/handoffs/` archives. Remaining root `notion-handoff-*.md` files mean unconsumed tickets.',
    '',
    '---',
    '',
    promptText.trimEnd(),
